...
7. **暫停** → 預設為F11，可自行於熱鍵設定變更
8. **偏移位置修正** → 需要先匡選目前所在小地圖，設置移動邊界後再進入位置修正加入移動用熱鍵，其餘按照說明設定即可
...
#### 位置修正系統重構
* **F7 邊界設定熱鍵：** 狀態機依序設定左→右→上→下邊界，操作更直覺
* **阻塞式修正：** 修正期間暫停腳本播放，避免按鍵衝突
* **時間補償：** 修正耗時自動從腳本計時中扣除，不影響播放節奏
* **水平/垂直獨立容差：** `HorizontalTolerance` / `VerticalTolerance` 分別設定，修正更精準
* **定期位置檢查：** 每 N 秒自動比對當前位置與腳本錄製座標，偏差超過閾值自動修正
* **設定即時套用：** 修改修正設定後不需重啟播放，下個循環自動生效
* **每次修正步數上限：** 可限制單次修正的按鍵次數，避免過度修正
* **按鍵間隔設定：** 可調整修正時每次按鍵的間隔（避免硬直）

#### 按鍵發送修正
* **修正器按鍵路由修正：** `SendKeyForCorrection` 改為跟隨使用者選擇的方向鍵模式（S2C/TAB/SWB），不再硬編碼
* **自定義按鍵前景洩漏修正：** 非方向鍵改用純 `PostMessage` 發送，不再透過 `AttachThreadInput` 洩漏到前景應用程式

#### 其他改善
* **F8 錄製過濾：** 錄製時自動過濾全局熱鍵（F7~F11），不會錄入熱鍵本身
* **漂移追蹤節流：** 位置追蹤改為每 2 秒一次，避免頻繁讀取拖慢腳本速度
* **回程方式更改：** 更改為點擊固定目標後按Enter回程

### v1.1.0
* 初始版本：錄製/播放、背景模式、自定義按鍵、排程系統、小地圖校準
